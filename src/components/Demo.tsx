...
function Demo() {
    ...
    useEffect(() => {
        let serializedArticle: string = localStorage.getItem('articles') || "{}";
        const articlesFromLocalStorage = JSON.parse(serializedArticle);
                ...
                <div className="flex flex-col w-full gap-2">
                    ...
                    <div className="flex flex-col gap-1 max-h-60 overflow-y-auto">
                        {allArticles.map((item, index) => (
                            <div key={`link-${index}`} onClick={() => {setArticle(item); console.log(article)}} className="link_card">
                                <div className="copy_btn" onClick={() => handleCopy(item.url)}>
                                    <img src={copied === item.url ? tick : copy} alt="copy icon" className="w-[40%] h-[40%] object-contain text-black" />
                                </div>
